...
client.on('message', async (message) => {
    ...
    if (message && message.content) {
        ...
        if (command === 'queuetime' && context) {
            message.channel.send(`Looking stats for ${context}, this may take a while...`);
...
const getNewWorldStatusHtml = async (serverName: string): Promise<IServerStatus | null> => {
    ...
    const stuff = await page.evaluate(() => {
        let rows: any[] = [];
        window.document.querySelectorAll("table#db76b9e516bd > tbody > tr").forEach(rowNode => { const el: (string | null)[] = []; rowNode.querySelectorAll('td').forEach(cell => el.push(cell.textContent)); rows.push(el) })
        return rows;
    });
